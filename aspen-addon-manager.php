...
final class Aspen_Addon_Manager {
	...
	private function is_eligible_for_cart_based_access( $product ) {
		...
		$this->eligible_cache[ $product_id ] = $this->is_memberships_purchase_restricted( $product_id ) && $this->product_has_matching_rule( $product_id ) && $this->cart_contains_qualifying_product( $product_id );
		return $this->eligible_cache[ $product_id ];
	}
